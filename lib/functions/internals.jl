...
model_summary(N::SpeciesInteractionNetwork{<:Partiteness, <:Binary})
    ...
    returns the 'summary statistics' for a network
"""
function model_summary(network::SpeciesInteractionNetwork{<:Partiteness, <:Binary},
    network_id::Any,
    model_name::String;
    ...
    if model_name ∉ ["random", "niche", "cascade", "hierarchy", "maxent", "neutral", "adbm"]
        error("Invalid value for model_name -- must be one of random, niche, cascade, hierarchy, maxent, neutral, adbm")
    end
